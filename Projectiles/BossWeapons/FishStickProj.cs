...
namespace FargowiltasSouls.Projectiles.BossWeapons
{
    public class FishStickProj : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            else
            {
                Main.projectile.Where(x => x.active && x.type == mod.ProjectileType("Whirlpool")).ToList().ForEach(x =>
                {
                    if (Main.rand.Next(2) == 0)
                    {
                        ...
                        Projectile shark = Projectile.NewProjectileDirect(x.Center, velocity, ProjectileID.MiniSharkron, projectile.damage / 2, projectile.knockBack, projectile.owner);
                        shark.tileCollide = false;
                        shark.timeLeft = 120;
                    }
                });
            }
            ...
            for (int i = 0; i < 20; i++)
            {
                int dust = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 59, -projectile.velocity.X * 0.2f, -projectile.velocity.Y * 0.2f, 100, default(Color), 2f);
                Main.dust[dust].noGravity = true;
                Main.dust[dust].velocity *= 2f;
                dust = Dust.NewDust(new Vector2(projectile.Center.X, projectile.Center.Y), projectile.width, projectile.height, 59, -projectile.velocity.X * 0.2f, -projectile.velocity.Y * 0.2f, 100);
                Main.dust[dust].velocity *= 2f;
            }
        }
    }
