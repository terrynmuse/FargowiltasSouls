		...
		public override void AI()
		{
			...
			if (projectile.ai[1] == 0f)
			{
				for (int num171 = 0; num171 < 200; num171++)
				{
					...
					{
						float num172 = Main.npc[num171].position.X + Main.npc[num171].width / 2;
						float num173 = Main.npc[num171].position.Y + Main.npc[num171].height / 2;
						float num174 = Math.Abs(projectile.position.X + projectile.width / 2 - num172) + Math.Abs(projectile.position.Y + projectile.height / 2 - num173);
						if (num174 < num169 && Collision.CanHit(new Vector2(projectile.position.X + projectile.width / 2, projectile.position.Y + projectile.height / 2), 1, 1, Main.npc[num171].position, Main.npc[num171].width, Main.npc[num171].height))
						{
							num169 = num174;
							num167 = num172;
							num168 = num173;
