        ...
        public override void SetDefaults(NPC npc)
        {
            if (FargoWorld.MasochistMode)
            {
                ...
                switch (npc.type)
                {
                    ...
                    case NPCID.CultistDragonTail: masoHurtAI = 6; break;
            ...
            if (FargoWorld.MasochistMode)
            {
                ...
                switch (masoAI)
                {
                        ...
                        else
                        {
                            ...
                            switch ((int)npc.ai[3]) //laser code idfk
                            {
                                ...
                                case 1: //slowing down, beginning rotation
                                    npc.velocity *= 1f - (npc.ai[0] - 3f) / 120f;
                                    npc.localAI[1] = 0f;
                                    npc.ai[2]--; //negate vanilla counting up
                                    npc.ai[2] -= (npc.ai[0] - 3f) / 120f * rotationInterval * (masoBool[2] ? 1f : -1f);
                                    npc.rotation = -npc.ai[2];

                                    if (npc.ai[0] >= 123f) //FIRE LASER
                                    {
                                        if (Main.netMode != 1)
                                        {
                    ...
                    case 35: //wall of flesh mouth
                        ...
                        if (!masoBool[0]) //when spawned in, make one eye invul
                        {
                            for (int i = 0; i < 200; i++)
                            {
                    ...
                    case 36: //destroyer head
                        ...
                        else
                        {
                            if (npc.HasPlayerTarget && !Main.dayTime && !Main.player[npc.target].dead)
                            {
                                ...
                                const float num14 = 17f;    //max speed?
                                const float num15 = 0.12f;   //turn speed?
                                const float num16 = 0.17f;   //acceleration?
                        ...
                        switch ((int)npc.ai[0])
                        {
                            ...
                            case 9: //phase 3 transition
                                ...
                                if (npc.ai[2] >= 120)
                                {
                                    ...
                                    {
                                        Counter = 0;
                                        int heal = npc.lifeMax * Main.rand.Next(100, 121) / 2000;
                                        npc.life += heal;
                                        if (npc.life > npc.lifeMax / 2)
                            ...
                            case 10: //phase 3
                                ...
                                npc.damage = (int)(npc.defDamage * 1.2 * (Main.expertMode ? 0.6f * Main.damageMultiplier : 1f));
                                npc.defense = npc.defDefense;
                                npc.position += npc.velocity * 0.25f;
                                    ...
                                    case 2: //magic
                                        for (int i = 0; i < 3; i++)
                                        {
                                            ...
                                            if (bodyPart.active &&
                                                ...
                                                bodyPart.type == NPCID.MoonLordHand))
                                            {
                                                Vector2 speed = new Vector2(0, -8f);
                                                speed = speed.RotatedBy(MathHelper.ToRadians(-60));
                                                for (int j = 0; j < 4; j++)
                                                {
                                                    Projectile.NewProjectile(bodyPart.Center, speed, ProjectileID.NebulaBolt, 45, 0f, Main.myPlayer, 0f, npc.whoAmI);
                                                    speed = speed.RotatedBy(MathHelper.ToRadians(40));
                                                }
                                            }
                                        }
                                        break;
                                    case 3: //summoner
                                        for (int i = 0; i < 3; i++)
                                        {
                                                ...
                                                bodyPart.type == NPCID.MoonLordHand))
                                            {
                                                ...
                                                for (int j = 0; j < 7; j++)
                                                {
                                                    int n = NPC.NewNPC((int)bodyPart.Center.X, (int)bodyPart.Center.Y, NPCID.AncientLight, 0, 0f, (Main.rand.NextFloat() - 0.5f) * 0.3f * 6.28318548202515f / 60f, speed.X, speed.Y);
                                                    if (n < 200)
                                                    {
                                                    ...
                                                    speed = speed.RotatedBy(Math.PI / 7);
                                                }
                                            }
                                        }
                                        break;
            ...
            if (FargoWorld.MasochistMode)
            {
                switch (npc.type)
                {
                    ...
                    case NPCID.CultistDragonHead:
                        target.AddBuff(mod.BuffType<FlamesoftheUniverse>(), Main.rand.Next(30, 90));
